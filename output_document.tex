...
českém matematikovi Otakaru Borůvkovi. Algoritmus poprvé popsal Borůvka v roce 1926 a byl 
...
\subsection{Rozdíl mezi Primovím algoritmem a Jarníkovím algoritmem}
Jarníkův algortimus je starší verze Primova algortimu. Vyvinul jí český matematik Vojtěch Jarník v roce 1930,
avšak poprvé byla publikována upravená verze algoritmu Robertem C. Primem, který algortimus oběvil a upravil v roce
1957. Aby toho nebylo málo, tak byl tento algortimus poté vylepšen v roce 1959 Edsgerem Dijkstrou, tudíž můžeme různé
...
Oba algortimy řeší problém nalezení minimální kostry grafu. Jedná se o problém, kdy chceme v nedirektovaném
vyváženém grafu najít nejmenší možnou kostru. To znamená že máme graf, ve kterém jsou náhodně propojené prvky.
Každý propoj má nějakou hodnotu. Naším cílem je najít stromovou strukturu, díky které se dostateneme ke každému "nodu"
...
vyhledávat data, či ji jakkoliv procházet. Jako příklad bychom mohly využít routování v síti, kdy si router najde
všechny zařízení na síti pomocí ARP protocolu, zjistí response time pro jednotlivé zařízení za pomocí pingu, 
sestaví nedirektovaný vyvážený graf, poté využije nějaký algoritmus na najdutí minimální kostry grafy a 
dle toho poté posílá packety. S největší pravděpodobností v dnešní době daný router nebude používat tento starý 
algortimus, ale teoreticky bychom ho mohly najít na velice starých modelech. Algoritmus je na úpravu dat
do podoby, která se poté hodí a zrychluje jiné algoritmy.
...
\pagebreak
\subsection{Borůvkův algoritmus}
Jedná se o standardní implementaci Borůvkova algortimu, která byla vytvořena dle pseudo-kód nalezeného níže.
Algortimus je psaný (bohužel) v C\#, jelikož jsem po 6 hodinách C již neměl síly a segmentation fault jsem viděl
při každém mrknutí :D.
...
Tento proces pokračuje, dokud není graf úplně propojen, čímž vznikne minimální koster.
